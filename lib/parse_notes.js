...
async function parseNotes(directoryPath, logger) {
  ...
  log.verbose(`Found ${files.length} markdown files in ${directoryPath}.`);
  log.debug(`Found these files in ${directoryPath}: ${files}`);

  const permalinks = files.map(file => {
    return file.replace(/\.[^/.]+$/, "");
  });
  log.verbose(`Found ${permalinks.length} permalinks in notes.`);

  files.forEach(filename => {
    fileParsers.set(filename, getFileParser(path.resolve(directoryPath, filename), permalinks));
  });

  log.verbose("Number of file parsers: " + fileParsers.size);
